...
from crypticlog.models import Admin, Post, Category, Comment
...
class CLITestCase(BaseTestCase):
    ...
    def test_init_command(self):
        result = self.runner.invoke(args=['init', '--username', 'fancy', '--password', '123'])
        ...
        self.assertEqual(Admin.query.first().username, 'fancy')
        self.assertEqual(Category.query.first().name, 'Default')

    def test_init_command_with_update(self):
        self.runner.invoke(args=['init', '--username', 'fancy', '--password', '123'])
        result = self.runner.invoke(args=['init', '--username', 'new fancy', '--password', '123'])
        ...
        self.assertEqual(Admin.query.count(), 1)
        self.assertEqual(Admin.query.first().username, 'new fancy')
        self.assertEqual(Category.query.first().name, 'Default')
